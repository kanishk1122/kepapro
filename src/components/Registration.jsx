    ...
    <div className="bg-neutral-900 text-white">
      <div className="h-screen flex justify-center items-center ">
        <div className={` ${scale ? "w-full ,z-1, h-full " : " w-10 , z-1 "} flex justify-center items-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}>
          ...
          <Link to="/" className="bg-white absolute top-[70vh] h-fit w-fit px-2 py-1 rounded-full text-black font-bold text-2xl font-['monument']">home</Link>
        </div>
        <div className={`w-[340px]  ${scale ? "hidden" : null } duration-700 rounded-3xl p-4 backdrop-blur-sm ${scale ? "opacity-0" : "opacity-100"}  h-3/4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[rgba(60,58,58,1)]`}>
          <form className={` flex justify-center   flex-col gap-8 items-center`} onSubmit={handleSubmit}>
            ...
            <div className="flex ">
             ...
             <h1>Enter password</h1>
              ...
              <div onClick={() => setPasswordVisible(!passwordVisible)} className="  duration-500">
                {passwordVisible ? (
            ...
            <input type="submit" className="bg-blue-900 px-2 py-1 rounded" onClick={() => setScale(!scale)} value={"Create an account"} />
