  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 404) {
        setflash("Email not found. Please check your email address.");
      } else if (error.response && error.response.status === 401) {
        setflash("Incorrect password. Please try again.");
      } else {
        setflash("Something went wrong. Please try again later.");
      }
    } finally {
      setIsLoading(false);
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input
                    type="checkbox"
                    className="w-4 h-4 rounded border-zinc-700 bg-zinc-900 text-red-600 focus:ring-red-500 focus:ring-offset-0 cursor-pointer"
                  />
